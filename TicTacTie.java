/**Simulation of a Tic-Tac-Tie game (does not do strategy)*/
package tictactoe;
public class TicTacTie {
	...
	/**Constructor*/
	public TicTacTie() { clearBoard(); }
	/**Clears the board*/
	public void clearBoard() {
		for (int i=0;i<3;i++)
			for (int j=0;j<3;j++)
				board[i][j] = EMPTY;				// every cell should be empty
		player = X;									// the first player in 'X
	}
	...
	/**Returns the winning playing's code,or 0 to indicate a tie(or unfinished game)*/
	public int winner() {
		if(isWin(X))
			return(X);
		else if(isWin(O))
	...
	/**Return a simple character string showing the current board.*/
	...
	public static void main(String[] args) {
		...
		String[] outcome = {"0 wins", "Tie", "X wins"}; // rely on order
